            ...
            foreach (var actionParameter in context.ApiDescription.ActionDescriptor.Parameters)
            {
                var properties =
                    actionParameter.ParameterType.GetProperties()
                        .Where(p => p.PropertyType == typeof(IFormFile) || p.PropertyType.GetGenericTypeDefinition() == typeof(LargeFormFile<>))
                        .Select(p => p.Name)
                        .ToArray();
                ...
                if (actionParameter.ParameterType == typeof(IFormFile)
                    || actionParameter.ParameterType.GetGenericTypeDefinition() == typeof(LargeFormFile<>))
                {
